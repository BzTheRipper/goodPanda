...
export const MessagetestPage = () => {
    ...
    useEffect(() => {
        ...
        let interval = setInterval(() => {
            ...
            socket.emit("user-message", { commands: Array.from(combined), speed });
        }, 100);
    ...
    return (
        <div className="h-[100dvh] w-full bg-black flex flex-col items-center touch-none overflow-hidden select-none relative">
            ...
            {/* FPV BACKGROUND (Optimized for Mobile) */}
            <div className="absolute inset-0 z-0 flex items-center justify-center bg-[#050a05] h-full w-full">
                {primaryLink ? (
                    <img 
                        src={primaryLink} 
                    ...
                    <div className="flex flex-col items-center gap-2">
                        <Circle className="size-8 text-emerald-500 animate-ping opacity-20" />
                        <p className="text-emerald-500/40 font-black tracking-widest text-[10px] uppercase">Waiting for FPV...</p>
                    </div>
                )}
            ...
            <div className="absolute inset-0 z-10 flex flex-col pointer-events-none">
                ...
                {isMobile && (!isLandscape || !isFullScreen) && (
                    <div className="fixed inset-0 z-[100] bg-[#050a05] flex flex-col items-center justify-center text-center p-6 text-white font-bold pointer-events-auto">
                        ...
                        <button onClick={enterLandscapeConsole} className="bg-[#2dd4bf] text-black font-bold py-2 px-6 rounded-full text-sm">START</button>
                    </div>
                )}
                ...
                <header className="w-full py-1 lg:py-2 flex justify-center items-center bg-black/40 backdrop-blur-md border-b border-white/10 pointer-events-auto">
                    ...
                    <div className="absolute right-4 flex items-center gap-2">
                        <div className={`size-2 lg:size-3 rounded-full ${isArmedFromTel ? "bg-emerald-500 animate-pulse" : "bg-red-600 animate-pulse"}`} />
                    </div>
                </header>

                {/* SPEED SIDEBAR - Moved to extreme right to prevent joystick overlap */}
                <div className="absolute right-1 lg:right-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-1 z-50 pointer-events-auto">
                    <span className="text-[8px] lg:text-[10px] text-[#2dd4bf] font-bold font-mono">{speed}%</span>
                    <div className="relative w-6 h-32 lg:w-10 lg:h-64 bg-black/60 border border-white/10 rounded-full flex flex-col-reverse p-0.5">
                ...
                <main className="flex-1 w-full flex flex-row items-end justify-between px-2 pb-2 lg:px-12 lg:pb-12">
                    {/* Left Joystick */}
                    <div className="w-1/4 flex justify-start pointer-events-auto">
                    ...
                    {/* CENTER HUD - Optimized Size for Mobile */}
                    <div className="flex-1 flex flex-col items-center gap-2 mb-2 pointer-events-auto max-w-[140px] lg:max-w-xs">
                        <div className='bg-black/80 backdrop-blur-lg border border-emerald-500/20 p-1.5 lg:p-2 rounded-lg shadow-2xl w-full'>
                            <div className="text-center mb-1 border-b border-white/5 pb-0.5">
                                <p className={`text-[7px] lg:text-[10px] font-black uppercase ${isArmedFromTel ? "text-emerald-400" : "text-red-500"}`}>
                        ...
                        <button onClick={handleForceDisarm} className="w-full py-1.5 bg-red-600/20 border border-red-500/40 rounded-lg text-red-500 font-black text-[8px] lg:text-[10px] uppercase active:bg-red-600 active:text-white transition-all">FORCE KILL</button>
                        ...
                        <div ref={sliderRef}
                            ...
                            className="relative w-10 h-28 lg:w-16 lg:h-56 bg-black/60 rounded-2xl border border-white/10 flex items-center justify-center backdrop-blur-sm"
                        >
                            <div className="absolute top-1 text-[5px] lg:text-[8px] font-bold text-emerald-500 opacity-40">ARM</div>
                            <div className="absolute bottom-1 text-[5px] lg:text-[8px] font-bold text-orange-500 opacity-40">LAND</div>
                            <div className="absolute w-7 h-7 lg:w-14 lg:h-14 flex items-center justify-center shadow-2xl transition-all" style={{ transform: `translateY(${yPos}px)`, borderRadius: '50%', background: isArmedFromTel ? "#10b981" : "#06b6d4" }}>
                                <Power size={isMobile ? 14 : 18} />
                            </div>
                        </div>
                    </div>

                    {/* Right Joystick (Padded to avoid Slider) */}
                    <div className="w-1/4 flex justify-end pr-6 lg:pr-0 pointer-events-auto">
                        <Joystick onMove={(dx, dy, r) => {
                            const t = r * 0.3; const d = new Set();
