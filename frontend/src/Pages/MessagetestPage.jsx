    ...
    return (
        <div className="relative flex flex-col items-center pointer-events-auto">
            ...
            >
                ...
                <div ref={stickRef} className="w-8 h-8 lg:w-14 lg:h-14 rounded-full shadow-2xl flex items-center justify-center bg-[#2dd4bf] pointer-events-none will-change-transform">
                    <div className="w-2 h-2 lg:w-3 lg:h-3 rounded-full border border-black/10" />
                </div>
            </div>
        </div>
    );
...
export const MessagetestPage = () => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            const combinedCommands = new Set([...Array.from(activeKeys.current), ...Array.from(leftJoyDirs.current), ...Array.from(rightJoyDirs.current)]);
            socket.emit("user-message", { commands: Array.from(combinedCommands), speed: speed });
        }, 100);
    ...
    return (
        <div className="h-[100dvh] w-full bg-black flex flex-col items-center touch-none overflow-hidden select-none relative">
            ...
            {/* LAYER 0: FPV BACKGROUND - Fixed for Mobile */}
            <div className="absolute inset-0 z-0 flex items-center justify-center bg-[#050a05] w-full h-full overflow-hidden">
                {primaryLink ? (
                    <img 
                        src={primaryLink} 
                        alt="Drone FPV" 
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-emerald-500/40 font-black tracking-widest text-sm uppercase">Connecting FPV...</p>
                    </div>
                )}
            ...
            <div className="absolute inset-0 z-10 flex flex-col pointer-events-none">
                ...
                <header className="w-full py-1 lg:py-2 flex justify-center items-center bg-black/40 backdrop-blur-md border-b border-white/10 pointer-events-auto">
                    ...
                    {primaryLink && !isFPVActive && (
                        <button onClick={activateFPV} className="absolute left-6 flex items-center gap-2 px-3 py-1 bg-emerald-500 text-black rounded-full text-[8px] font-bold animate-bounce shadow-lg">
                    ...
                    <div className="absolute right-6 flex items-center gap-2">
                        ...
                        <div className={`size-2 lg:size-3 rounded-full border border-white/10 shadow-lg ${isArmedFromTel ? "bg-emerald-500 animate-pulse shadow-emerald-500/50" : "bg-red-600 animate-pulse shadow-red-500/50"}`} />
                    </div>
                </header>

                {/* SPEED CONTROL SIDEBAR - Shifted for mobile thumb safety */}
                <div className="absolute right-2 lg:right-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-1 lg:gap-2 z-50 pointer-events-auto">
                    ...
                    <div className="relative w-6 h-32 lg:w-10 lg:h-64 bg-black/60 border border-white/10 rounded-full flex flex-col-reverse p-1 overflow-hidden">
                ...
                <main className="flex-1 w-full flex flex-row items-end justify-between px-2 pb-2 lg:px-12 lg:pb-12">
                    <div className="flex-1 flex justify-start pointer-events-auto">
                        <div className="flex flex-col items-center mb-2 lg:mb-10 w-fit">
                            <p className="text-[8px] text-emerald-500/40 font-bold mb-2 uppercase tracking-widest italic">Direction</p>
                    ...
                    <div className="flex flex-row items-center gap-4 lg:gap-10 mb-4 shrink-0 z-10 w-fit pointer-events-auto">
                        <div className="flex flex-col gap-2 lg:gap-3 w-28 lg:w-80">
                            ...
                            <div className='bg-black/80 backdrop-blur-lg border border-emerald-500/20 p-1.5 lg:p-2 rounded-xl shadow-2xl w-full'>
                                <div className="text-center mb-1 border-b border-white/5 pb-1">
                                    <p className={`text-[8px] lg:text-[10px] font-black uppercase ${isArmedFromTel ? "text-emerald-400" : "text-red-500 animate-pulse"}`}>
                                        {gotTheTelMessage?.theTelMessage?.status_msg || "CONNECTING..."}
                                    </p>
                                </div>
                                <div className="grid grid-cols-3 text-center text-[#2dd4bf] font-mono text-[8px] lg:text-[14px]">
                                    <div><p className="text-gray-500 text-[5px] lg:text-[6px]">X</p>{gotTheTelMessage?.theTelMessage?.x?.toFixed(1) || "0.0"}</div>
                                    <div><p className="text-gray-500 text-[5px] lg:text-[7px]">Y</p>{gotTheTelMessage?.theTelMessage?.y?.toFixed(1) || "0.0"}</div>
                                    <div><p className="text-gray-500 text-[5px] lg:text-[6px]">θ</p>{gotTheTelMessage?.theTelMessage?.theta?.toFixed(1) || "0.0"}</div>
                                </div>
                            </div>

                            <button onClick={handleForceDisarm} className="w-full py-1.5 lg:py-2 bg-red-600/20 border border-red-500/50 rounded-xl flex items-center justify-center gap-2 text-red-500 font-black text-[8px] lg:text-[10px] uppercase hover:bg-red-600 hover:text-white transition-all active:scale-95 shadow-2xl">
                                <ShieldAlert size={14} /> FORCE KILL
                            </button>
                        </div>

                        {/* SLIDER - NO POSITION CHANGE */}
                        <div ref={sliderRef}
                            ...
                            className="relative w-10 lg:w-12 h-36 lg:h-56 bg-black/60 rounded-3xl border border-white/10 flex items-center justify-center backdrop-blur-sm"
                        >
                            <div className="absolute top-2 text-[5px] lg:text-[6px] font-bold text-emerald-500 opacity-40 uppercase">Arm</div>
                            <div className="absolute bottom-2 text-[5px] lg:text-[6px] font-bold text-orange-500 opacity-40 uppercase">Land</div>
                            <div className="w-full h-0.5 bg-white/10 absolute" />
                            <div className="absolute w-8 h-8 lg:w-10 lg:h-10 flex items-center justify-center shadow-2xl transition-all duration-150" style={{ transform: `translateY(${yPos}px)`, borderRadius: '50%', background: isArmedFromTel ? "#10b981" : "#06b6d4" }}>
                                <Power size={18} />
                            </div>
                        </div>
                    </div>

                    {/* Right Stick Area - Added PADDING to separate from Throttle Sidebar */}
                    <div className="flex-1 flex justify-end pr-10 lg:pr-0 pointer-events-auto">
                        <div className="flex flex-col items-center mb-2 lg:mb-10 w-fit">
