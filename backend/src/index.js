...
io.on("connection", async (connectedClient) => {
  ...
  if (userId) {
    ...
    try {
      ...
      if (user) {
        ...
        console.log(`⚠️ userId ${userId} provided but not found in Database.`);
      }
    } catch (err) {
      console.log("❌ Socket user fetch error:", err.message);
    }
  }
  ...
  connectedClient.on("user-message", async (message) => {
    ...
    const senderName = connectedClient.user?.fullname || "Unknown";
    io.emit("message", { name: senderName, id: userId, theMessage: message });
